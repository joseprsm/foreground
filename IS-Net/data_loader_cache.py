    ...
    for i in range(len(datasets)):
        ...
        if flag == "train":  # combine multiple training sets into one dataset
            if len(name_im_gt_list) == 0:
                ...
                name_im_gt_list[0]["dataset_name"] = "_".join(
                    [name_im_gt_list[0]["dataset_name"], datasets[i]["name"]]
                )
...
def create_dataloaders(
    ...
    cache_size = cache_size or []
    my_transforms = my_transforms or []
    gos_dataloaders = []
    gos_datasets = []
        ...
        for i in range(0, len(name_im_gt_list)):
            ...
            dt_name_list.extend(
                [
                    ...
                    for _ in name_im_gt_list[i]["im_path"]
                ]
            )
            im_name_list.extend(
                [
